...
class RoboTwinVLADataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            valid_timesteps_path: Path to JSON file with valid (non-idle) timesteps per episode.
                Generated by compute_idle_masks.py. Required for dataset construction.
        ...
        # Load valid timesteps (required)
        valid_timesteps_file = Path(valid_timesteps_path)
        if not valid_timesteps_file.exists():
            raise FileNotFoundError(
                f"Valid timesteps file not found at {valid_timesteps_file}\n"
                f"Please run compute_idle_masks.py first to generate this file."
            )

        print(f"Loading valid timesteps from {valid_timesteps_file}...")
        with open(valid_timesteps_file) as f:
            valid_timesteps_raw = json.load(f)
        # Convert to sets for O(1) lookup during state history filtering
        self.valid_timesteps = {k: set(v) for k, v in valid_timesteps_raw.items()}
        total_valid_timesteps = sum(len(ts) for ts in self.valid_timesteps.values())
        print(f"  Loaded {total_valid_timesteps} valid timesteps from {len(self.valid_timesteps)} episodes")

        # Build list of valid (episode, timestep) pairs
        # Each episode contributes multiple samples (one per valid timestep)
        self.samples = []
        episodes_found = 0
        episodes_missing = 0
        ...
        for ep_meta in self.index.episodes:
            # Create unique episode key (must match compute_idle_masks.py format)
            episode_key = f"{ep_meta.task_name}/{ep_meta.robot_type}_{ep_meta.variant}/episode{ep_meta.episode_idx}"

            if episode_key not in self.valid_timesteps:
                episodes_missing += 1
                continue

            episodes_found += 1
            # Add all valid timesteps for this episode
            for t in self.valid_timesteps[episode_key]:
                self.samples.append((ep_meta, t))

        print(f"Total samples: {len(self.samples)}")
        print(f"  Episodes found in valid_timesteps: {episodes_found}")
        if episodes_missing > 0:
            print(f"  Episodes missing from valid_timesteps: {episodes_missing}")
